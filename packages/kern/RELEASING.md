...
`SECURITY.md` keeps email as a fallback. Before each major release, visit the repository's
**Security → Advisories** page from a non-admin account and verify that **Report a vulnerability**
opens the private reporting form. Do not disable private vulnerability reporting without first
replacing every direct reporting link with another working private channel.
