...
```ts
import { subtractMoney } from "@lithekit/money"
...
```ts
import { sumMoney } from "@lithekit/money"
...
```ts
import { multiplyMoney } from "@lithekit/money"
...
```ts
import { percentageOf } from "@lithekit/money"
...
```ts
import { applyDiscount } from "@lithekit/money"
...
Lithekit does not select a jurisdictional policy for you.

## Round to a cash increment

```ts
import { roundMoney } from "@lithekit/money"
...
```ts
import { allocateMoney } from "@lithekit/money"
...
5. Lithekit calculates exact proportions with `bigint`.
