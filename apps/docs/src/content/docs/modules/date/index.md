...
“day” and “month” are interpreted through a timezone. Lithekit makes that distinction visible instead
...
```ts
import { addDays, formatDate, isAfter, toUTCISODate } from "@lithekit/date"
...
- Lithekit does not parse date strings. Construct or validate the `Date` at your application boundary.
...
## What Lithekit deliberately does not do

Lithekit does not provide named-timezone arithmetic such as “add one business day in Europe/Paris,”
...
- `Date` is mutable, but Lithekit returns new dates and leaves supplied dates unchanged.
...
```ts
import { formatDateTime, isToday } from "@lithekit/date"
...
Chrome, and Node 26. Lithekit's Node 22/24 and Bun baselines still prevent universal reliance on it, so
...
Lithekit can still be used. No Temporal type or runtime dependency becomes part of the public API.
