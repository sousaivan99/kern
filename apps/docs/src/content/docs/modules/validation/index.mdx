...
forms, JSON, storage, and environment variables can still have the wrong shape. A Lithekit schema checks
...
```ts
import { enumeration, number, object, string } from "@lithekit/validation"
...
Lithekit does not generate or auto-import a separate interface. Most code simply uses the type of the
parsed return value.
...
5. [Validation benchmarks](../../measurements/validation-benchmarks/) records the equivalent Lithekit,
   Zod, and Valibot workloads, exact machine-local results, and reproduction protocol.
6. [Performance internals](../../advanced/performance-internals/) explains Lithekit's private fast and
   diagnostic paths, safety boundaries, and bundle-size tradeoffs.
...
```ts
import { type InferInput, type InferOutput, object, string } from "@lithekit/validation"
