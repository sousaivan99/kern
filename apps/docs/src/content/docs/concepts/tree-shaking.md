...
description: Import only what you use and understand Lithekit's enforced size limits.
---

Tree-shaking is a build step that removes exported code your application never uses. Lithekit is
...
## Why Lithekit can be tree-shaken

- Every module has an independent ESM entrypoint.
- The package declares `sideEffects: false`.
- Importing Lithekit does not register globals, patch prototypes, start timers, or read configuration.
...
Lithekit's build checks bundle, minify, gzip, and measure every public entrypoint.
...
5. Avoid namespace imports such as `import * as Lithekit` unless you genuinely use a dynamic namespace.
