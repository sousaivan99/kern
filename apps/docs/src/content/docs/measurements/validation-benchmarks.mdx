...
description: Run equivalent validation workloads against Lithekit, Zod, and Valibot with recorded methodology.
---

import BenchmarkEnvironments from "../../../components/BenchmarkEnvironments.astro";

The validation comparison records absolute timings for Lithekit, Zod, and Valibot. It does not assign
...
bun run benchmark:validation -- --json --library-order=Lithekit,Zod,Valibot \
  > .benchmarks/validation/run-1.json
bun run benchmark:validation -- --json --library-order=Zod,Valibot,Lithekit \
  > .benchmarks/validation/run-2.json
bun run benchmark:validation -- --json --library-order=Valibot,Lithekit,Zod \
...
`bun run benchmark:validation:lithekit` runs Lithekit's additional scaling and composition cases.
...
Capture the Lithekit-only scaling snapshot independently because it does not have a library order:

```bash
mkdir -p .benchmarks/reduction-release/validation-lithekit

bun run benchmark:validation:lithekit -- --json \
  > .benchmarks/reduction-release/validation-lithekit/run-1.json
bun run benchmark:validation:lithekit -- --json \
  > .benchmarks/reduction-release/validation-lithekit/run-2.json
bun run benchmark:validation:lithekit -- --json \
  > .benchmarks/reduction-release/validation-lithekit/run-3.json
...
Both snapshots compare Lithekit 1.0.0, Zod 4.4.3, and Valibot 1.4.2 through the public APIs represented
...
| Successful headline | CPU | Lithekit | Zod | Valibot | Lithekit advantage over fastest competitor |
| --- | --- | ---: | ---: | ---: | ---: |
...
The phase gate required Lithekit latency to be at most 90% of the lowest competitor latency. All five
...
Throughput multiples are the inverse of latency: fastest competitor latency divided by Lithekit
...
| Failure workload | CPU | Lithekit | Zod | Valibot | Interpretation |
| --- | --- | ---: | ---: | ---: | --- |
...
not assigned a synthetic implementation or excluded only from Lithekit's timing.
...
same path safely preserves an own `__proto__` key. These Lithekit-only results include field validation
...
The stable IDs are `validation-lithekit:record-success:1000`,
`validation-lithekit:record-success:10000`, and `validation-lithekit:record-success:100000`.
...
A separate single Node 24.16 guard run recorded Lithekit at 102.6 ns for three-field `parse()`, 116.2 ns
...
| --- | --- | --- | --- |
| 1 | Lithekit → Zod → Valibot | 2026-08-16 16:05:32 | `2b6465dcad9589741c8274cd4489eb381d4014552e0f36c4a6ed17db3c61add9` |
| 2 | Zod → Valibot → Lithekit | 2026-08-16 16:06:16 | `9e853fe752ac26a4e48f4937771723922c65a55c144162b4f7ae1141189a12ce` |
| 3 | Valibot → Lithekit → Zod | 2026-08-16 16:06:58 | `0eb7b2a7e164e5d7f6d0987b6e25061c71b9be5c857aeb669782fcc353b52e8e` |
...
| --- | --- | --- | ---: | --- |
| 1 | Lithekit → Zod → Valibot | 2026-08-17 06:50:28 | 4,049 MHz | `a27097d147a2a655bcf5099936c0b69411e340d4c3099e2883431e4ace8dd819` |
| 2 | Zod → Valibot → Lithekit | 2026-08-17 06:51:12 | 4,377 MHz | `a131f5a545235f8569c7c43fa2994e343728f8a30e19d2b6b03d67c30c56822e` |
| 3 | Valibot → Lithekit → Zod | 2026-08-17 06:51:57 | 5,471 MHz | `ccf66a23b36ae57dd784026acfe7dadb2970d784ca015513dcc05d9d60a5d9f0` |
...
[validation comparison source](https://github.com/sousaivan99/lithekit/blob/develop/tooling/benchmarks/validation-compare.bench.ts).
...
- A scenario is excluded for every library when any library cannot run it. Zod's unsupported
  early-abort case therefore gives no points to Lithekit or Valibot.
...
- Rotate library execution order across three runs and compare the median of their medians. The
  accepted order is Lithekit/Zod/Valibot, Zod/Valibot/Lithekit, then Valibot/Lithekit/Zod.
