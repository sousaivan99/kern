...
const timeZones = ["UTC", "America/New_York", "Europe/Luxembourg", "Australia/Lord_Howe"] as const
const repositoryRoot = resolve(import.meta.dir, "../../..")
const dateRoot = join(repositoryRoot, "packages", "date")
...
for (let index = 0; index < timeZones.length; index += 1) {
  ...
  const result = await runCaptured(
    [process.execPath, "test", "tests/date.test.ts", "tests/date.property.test.ts"],
    { cwd: dateRoot, env: { ...process.env, TZ: timeZone } },
  )
