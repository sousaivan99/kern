const runtimeArguments = globalThis.process?.argv?.slice(2) ?? globalThis.Deno?.args ?? []
const workingDirectory = globalThis.process?.cwd?.() ?? globalThis.Deno?.cwd?.()
if (!workingDirectory) throw new Error("Unable to resolve the runtime working directory")
const roots = Object.fromEntries(
  runtimeArguments.map((directory) => {
    const absolute = directory.startsWith("/") ? directory : `${workingDirectory}/${directory}`
    const url = new URL(`file://${absolute.replace(/\/$/u, "")}/`)
    return [url.pathname.split("/").filter(Boolean).at(-2), url]
  }),
)

const { chunk, first, groupBy, last, partition, unique, uniqueBy, withoutFalsy } = await import(
  new URL("index.js", roots.array)
)
const { once, sleep } = await import(new URL("index.js", roots.async))
const {
  addDays,
  addMonths,
  addYears,
  ...
  startOfDay,
  subtractDays,
} = await import(new URL("index.js", roots.date))
const { allocateMoney, formatMoney, parseMoney, roundMoney, sumMoney } = await import(
  new URL("index.js", roots.money)
)
const { formatNumber, formatPercentage, isBetween, percentageOfTotal } = await import(
  new URL("index.js", roots.number)
)
const { deepFreeze, hasOwn, hasOwnPath } = await import(new URL("index.js", roots.object))
const { slugify, truncate } = await import(new URL("index.js", roots.string))
const { object, string } = await import(new URL("index.js", roots.validation))
